...
noncomputable def toComplex {n} {c : Fin n → realLorentzTensor.Color} :
  ...
  map_smul' c v := by
    simp only [map_smul, Finsupp.coe_smul, Pi.smul_apply, smul_eq_mul, Complex.ofRealHom_eq_coe,
      Finset.smul_sum, ← smul_smul, Complex.coe_smul]
  map_add' c v := by
...
lemma toComplex_repr {n} {c : Fin n → realLorentzTensor.Color}
  ...
  -- Expand `toComplex v` in the complexified basis; `repr` is linear so the sum collapses
  -- to the single term indexed by `i.complexify`.
  rw [toComplex_eq_sum_basis, map_sum, Finsupp.finsetSum_apply,
    Fintype.sum_eq_single i.complexify]
  · simp [← Complex.coe_smul]
  · intro k hk
    simp [← Complex.coe_smul, Ne.symm hk]
...
lemma toComplex_basis {n} {c : Fin n → realLorentzTensor.Color}
  ...
  refine (Fintype.sum_eq_single i fun j hj => ?_).trans ?_
  · simp [Ne.symm hj]
  · simp
...
lemma toComplex_pure_basisVector {n} {c : Fin n → realLorentzTensor.Color}
  ...
  rw [← Tensor.basis_apply (S := realLorentzTensor) (c := c) b, toComplex_basis (c := c) b,
    Tensor.basis_apply (S := complexLorentzTensor) (c := (colorToComplex ∘ c)) b.complexify]
...
noncomputable def toComplexVector (c : realLorentzTensor.Color) :
  ...
  map_add' v1 v2 := by
    ...
    · simp only [map_add, Finsupp.coe_add, Pi.add_apply, Nat.reduceAdd, ← Finset.sum_add_distrib]
      exact Finset.sum_congr rfl fun x _ => add_smul _ _ _
  map_smul' r v := by
    cases c <;>
    · simp only [map_smul, Finsupp.coe_smul, Pi.smul_apply, smul_eq_mul, Nat.reduceAdd,
        Complex.ofRealHom_eq_coe, Complex.coe_smul, Finset.smul_sum]
      exact Finset.sum_congr rfl fun x _ => (smul_smul _ _ _).symm
...
lemma toComplexVector_up_eq_inclCongrRealLorentz (v : Lorentz.ContrMod 3) :
  ...
  calc
    ...
    _ = Lorentz.inclCongrRealLorentz v := by
          conv_rhs => rw [Lorentz.ContrMod.stdBasis_decomp v, map_sum]
          simp only [Lorentz.complexContrBasis_of_real, LinearMap.map_smulₛₗ,
...
lemma toComplexVector_down_eq_inclCoRealLorentz (v : Lorentz.CoMod 3) :
  ...
  calc
    ...
    _ = Lorentz.inclCoRealLorentz v := by
          conv_rhs => rw [Lorentz.CoMod.stdBasis_decomp v, map_sum]
          simp only [Lorentz.complexCoBasis_of_real, LinearMap.map_smulₛₗ,
            Complex.ofRealHom_eq_coe, Complex.coe_smul]
    ...
    calc
      (Lorentz.ContrℂModule.SL2CRep Λ) ((toComplexVector Color.up) p)
          = (Lorentz.ContrℂModule.SL2CRep Λ) (Lorentz.inclCongrRealLorentz p) :=
            congrArg _ (toComplexVector_up_eq_inclCongrRealLorentz p)
      _ = Lorentz.inclCongrRealLorentz ((Lorentz.Contr 3).ρ (toLorentzGroup Λ) p) :=
    ...
    calc
      (Lorentz.CoℂModule.SL2CRep Λ) ((toComplexVector Color.down) p)
          = (Lorentz.CoℂModule.SL2CRep Λ) (Lorentz.inclCoRealLorentz p) :=
            congrArg _ (toComplexVector_down_eq_inclCoRealLorentz p)
      _ = Lorentz.inclCoRealLorentz ((Lorentz.Co 3).ρ (toLorentzGroup Λ) p) :=
            Lorentz.inclCoRealLorentz_ρ ..
...
lemma permT_toComplex {n m : ℕ}
  ...
  | h b =>
    -- permT on (real/complex) basis + toComplex on basis, then index equality.
...
lemma colorToComplex_append {n m : ℕ}
  ...
  funext x
  -- break `x : Fin (n+m)` into left/right parts; `simp` expands `Fin.append` on each.
...
lemma permCond_prodTColorToComplex {n m : ℕ}
  ...
  intro x
  -- exactly `colorToComplex_append` (with sides reversed) applied at `x`.
  simpa [Function.comp_apply] using congrArg (fun f => f x)
...
lemma prodT_toComplex {n m : ℕ}
  ...
  classical
  -- Double induction on both tensors using the tensor basis. We generalise `t1` so the
  -- outer motive quantifies over it.
  induction t using induction_on_basis generalizing t1 with
  | h b =>
    -- Inner induction on the second tensor.
    ...
    | hsmul r t1' ht' => simp [map_smul, ht', prodTColorToComplex]
    | hadd t1' t2' h1 h2 => simp [map_add, h1, h2, prodTColorToComplex]
  | hzero => simp [prodTColorToComplex]
  | hsmul r t ht => simp [map_smul, ht, prodTColorToComplex]
  | hadd ta tb h1 h2 => simp [map_add, h1, h2, prodTColorToComplex]
...
lemma contrT_toComplex {n : ℕ}
    ...
    contrT (S := complexLorentzTensor) n i j (by
      ...
      (toComplex (c := c) t) := by
  classical
  -- Induction on `t` using the tensor basis; the colour-transport proof obligation is
  -- captured automatically by `induction_on_basis`.
  induction t using induction_on_basis with
  | h b =>
    -- `(Tensor.basis c) b = (Pure.basisVector c b).toTensor`; equate both sides via
...
lemma evalT_toComplex {n : ℕ}
  ...
  | hzero => simp only [evalTColorToComplex, map_zero]
  | hsmul r t' ht' =>
    rw [LinearMap.map_smul, toComplex_map_smul (c ∘ i.succAbove) r
      ((evalT (S := realLorentzTensor) i b) t'), ht', toComplex_map_smul (c := c) r t']
