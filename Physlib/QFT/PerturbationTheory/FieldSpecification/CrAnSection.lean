...
def consEquiv {φ : 𝓕.FieldOp} {φs : List 𝓕.FieldOp} : CrAnSection (φ :: φs) ≃
  ...
  invFun ψψs :=
    match ψψs with
    | (ψ, ψs) => cons ψ ψs
  left_inv ψs := Subtype.ext (Eq.symm eq_head_cons_tail)
  right_inv ψψs := by
    match ψψs with
    | (ψ, ψs) => rfl
...
lemma card_nil_eq : Fintype.card (CrAnSection (𝓕 := 𝓕) []) = 1 := by
  simpa using Fintype.ofEquiv_card nilEquiv.symm
...
lemma sum_cons (f : CrAnSection (φ :: φs) → M) [AddCommMonoid M] :
  ...
  simpa [Fintype.sum_prod_type, consEquiv] using Eq.symm (consEquiv.symm.sum_comp f)
