...
lemma normalOrderSign_swap_annihilate_annihilate (φa φa' : 𝓕.CrAnFieldOp)
  ...
  | φ :: φs, φs' => by
    ...
    dsimp only [normalOrderSign, Wick.koszulSign]
    rw [← normalOrderSign]
    simpa only [mul_eq_mul_right_iff] using Or.inl (Wick.koszulSignInsert_eq_perm _ _ _ _ _
...
@[simp]
lemma normalOrderList_nil : normalOrderList (𝓕 := 𝓕) [] = [] := by simp [normalOrderList]

@[simp]
lemma normalOrderList_statistics (φs : List 𝓕.CrAnFieldOp) :
    (𝓕 |>ₛ (normalOrderList φs)) = 𝓕 |>ₛ φs := by simp [normalOrderList]
