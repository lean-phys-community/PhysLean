...
lemma vectorPotential_contDiff_of_smooth {n : ℕ} {d} {c : SpeedOfLight}
    ...
    ContDiff ℝ n ↿(A.vectorPotential c) :=
  vectorPotential_contDiff A (hA.of_le (ENat.LEInfty.out))

lemma vectorPotential_apply_contDiff {n} {d} {c : SpeedOfLight} (A : ElectromagneticPotential d)
    (hA : ContDiff ℝ n A) (i : Fin d) : ContDiff ℝ n ↿(fun t x => A.vectorPotential c t x i) :=
  (ContinuousLinearMap.contDiff (𝕜 := ℝ) (n := n) (EuclideanSpace.proj i)).comp
    (vectorPotential_contDiff A hA)
...
lemma vectorPotential_contDiff_space {n} {d} {c : SpeedOfLight} (A : ElectromagneticPotential d)
    (hA : ContDiff ℝ n A) (t : Time) : ContDiff ℝ n (A.vectorPotential c t) :=
  (vectorPotential_contDiff A hA).comp (f := fun x => (t, x)) (by fun_prop)

lemma vectorPotential_apply_contDiff_space {n} {d} {c : SpeedOfLight}
    (A : ElectromagneticPotential d)
    (hA : ContDiff ℝ n A) (t : Time) (i : Fin d) :
    ContDiff ℝ n (fun x => A.vectorPotential c t x i) :=
  (ContinuousLinearMap.contDiff (𝕜 := ℝ) (n := n) (EuclideanSpace.proj i)).comp
    (vectorPotential_contDiff_space A hA t)

lemma vectorPotential_contDiff_time {n} {d} {c : SpeedOfLight} (A : ElectromagneticPotential d)
    (hA : ContDiff ℝ n A) (x : Space d) : ContDiff ℝ n (A.vectorPotential c · x) :=
  (vectorPotential_contDiff A hA).comp (f := fun t => (t, x)) (by fun_prop)
...
lemma vectorPotential_differentiable_time {d} {c : SpeedOfLight} (A : ElectromagneticPotential d)
  ...
  (vectorPotential_differentiable A hA).comp (f := fun t => (t, x)) (by fun_prop)
