...
/-- Multiplication of transfer matrices, `MatrixMap.toMatrix`, equals composition of maps. -/
...
/-- Notation `M₁ ⊗ₖₘ M₂` for the Kronecker product `MatrixMap.kron M₁ M₂` of two matrix maps. -/
...
theorem kron_def [CommSemiring R] (M₁ : MatrixMap A B R) (M₂ : MatrixMap C D R)
  ...
  have h_expand : (Matrix.toLin (Matrix.stdBasis R (A × C) (A × C))
      ...
      = ∑ a₁ : A, ∑ a₂ : A, ∑ c₁ : C, ∑ c₂ : C, M (a₁, c₁) (a₂, c₂)
        • (Matrix.toLin (Matrix.stdBasis R (A × C) (A × C)) (Matrix.stdBasis R (B × D) (B × D)))
        ((Matrix.reindex (Equiv.prodProdProdComm B B D D) (Equiv.prodProdProdComm A A C C))
        ((LinearMap.toMatrix ((Matrix.stdBasis R A A).tensorProduct (Matrix.stdBasis R C C))
        ((Matrix.stdBasis R B B).tensorProduct (Matrix.stdBasis R D D)))
  ...
  simp only [Matrix.stdBasis,
    ...
    LinearMap.toMatrix_apply, Module.Basis.tensorProduct_apply, Module.Basis.map_apply,
    Module.Basis.coe_reindex, Function.comp_apply, Equiv.sigmaEquivProd_symm_apply,
    Pi.basis_apply, Pi.basisFun_apply, Matrix.coe_ofLinearEquiv, TensorProduct.map_tmul,
    Module.Basis.tensorProduct_repr_tmul_apply, Module.Basis.map_repr, LinearEquiv.trans_apply,
    Matrix.coe_ofLinearEquiv_symm, Module.Basis.repr_reindex, Finsupp.mapDomain_equiv_apply,
...
/-- For maps L₁, L₂, R₁, R₂: (L₂ ∘ₗ L₁) ⊗ₖₘ (R₂ ∘ₗ R₁) = (L₂ ⊗ₖₘ R₂) ∘ₗ (L₁ ⊗ₖₘ R₁). -/
