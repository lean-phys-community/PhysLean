  ...
  obtain ⟨v, hv⟩ : ∃ v : EuclideanSpace ℂ d₂, v ∉ (σ₂ :HermitianMat d₂ ℂ).ker ∧ v ∉ (ρ₂ :HermitianMat d₂ ℂ).ker := by
    have h_union : (σ₂ : HermitianMat d₂ ℂ).ker ≠ ⊤ ∧ (ρ₂ : HermitianMat d₂ ℂ).ker ≠ ⊤ := by
      ...
      · have h_contra : σ₂.M = 0 := by
          ...
          simp_all only [DensityOp.mat_M, EuclideanSpace.ofLp_single, Matrix.mulVec_single,
...
lemma ker_prod_le_iff (ρ₁ σ₁ : MState d₁) (ρ₂ σ₂ : MState d₂) :
  ...
  · simp only [HermitianMat.ker] at h ⊢
    exact ker_kron_le_of_le _ _ _ _ h.1 h.2
...
private theorem sandwichedRelRentropy_op_nonneg {α : ℝ} (hα : 0 < α) {ρ σ : DensityOp E}
    (h : σ.op.ker ≤ ρ.op.ker) :
...
theorem sandwichedRelRentropy_self (hα : 0 < α) (ρ : MState d) :
  ...
  --TODO: Maybe SandwichedRelRentropy should actually be defined differently for α = 0?
    D̃_ α(ρ‖ρ) = 0 := by
  simp? [MState.sandwichedRelRentropy_eq_matrix, NNReal.eq_iff, hα] says
    simp only [MState.sandwichedRelRentropy_eq_matrix, hα, ↓reduceDIte, le_refl, sub_self,
      inner_zero_right,
