...
/-!
Entanglement measures. (Mixed) convex roof extensions. Definition of product / separable /
entangled states are in `Braket.lean` and/or `MState.lean`
...
TODO:
 - Other entanglement measures (not necessarily based on convex roof extensions). In roughly
   ...
   [1] Christandl, Matthias. “The Structure of Bipartite Quantum States - Insights from Group
       Theory and Cryptography.”
       https://doi.org/10.48550/arXiv.quant-ph/0604183.
 - Define classes of entanglement measures with good properties, including: monotonicity under
   LOCC (easier: just LO), monotonicity on average under LOCC, convexity (if the latter two are
   present, it is called an entanglement monotone by some), vanishing on separable states,
   normalized on the maximally entangled state, (sub)additivity, regularisible.
 ...
 - Useful properties of convex roof extensions:
   ...
   2. If f ψ is zero if and only if ψ is a product state, then its convex roof is faithful: zero
      if and only if the mixed state is separable
...
for `le_iInf` and `iInf_le_of_le`. However, it is also proven in `convex_roof_ne_top` that the
convex roof is never `∞`, so the definition `convex_roof` should be used in most applications. -/
...
This is valued in the extended nonnegative real numbers `ℝ≥0∞` to have good properties of the
infimum, which come from the fact that `ℝ≥0∞` is a complete lattice (see
`ENNReal.instCompleteLinearOrder`). However, it is also proven in `mixed_convex_roof_ne_top` that
the mixed convex roof is never `∞`, so the definition `mixed_convex_roof` should be used in most
applications. -/
...
variable [Nonempty d] [Nonempty d₁] [Nonempty d₂]
...
`convex_roof_ENNReal`. Hence, it should be used in proofs alongside `convex_roof_ne_top`. -/
...
This is valued in the nonnegative real numbers `ℝ≥0` by applying `ENNReal.toNNReal` to
`mixed_convex_roof_ENNReal`. Hence, it should be used in proofs alongside
...
The function `Sᵥₙ ∘ traceRight ∘ pure` is phase-invariant because `pure` maps phase-equivalent
kets to the same mixed state, so it descends to `KetUpToPhase` via `pureQ`. -/
...
/-
The von Neumann entropy of a state is equal to the trace of `ρ log ρ`
(technically `cfc ρ negMulLog`).
-/
theorem Sᵥₙ_eq_trace_cfc {d : Type*} [Fintype d] [DecidableEq d] (ρ : MState d) :
    Sᵥₙ ρ = (HermitianMat.cfc ρ.M Real.negMulLog).trace := by
  -- By definition of von Neumann entropy, we have
  -- Sᵥₙ ρ = Finset.sum Finset.univ (fun x ↦ Real.negMulLog (ρ.M.H.eigenvalues x)).
...
/-
The von Neumann entropy of a classical state (diagonal in the basis) is equal to the Shannon
...
theorem Sᵥₙ_ofClassical {d : Type*} [Fintype d] [DecidableEq d] (dist : ProbDistribution d) :
  ...
  -- By definition of $MState.ofClassical$, we know that $(MState.ofClassical dist).M$ is a
  -- diagonal matrix with entries $dist i$.
