...
/-!
# Filter and limit lemmas

## i. Overview

Miscellaneous lemmas about filters and limits used elsewhere in the development, concerning
convergence of natural-number division ratios and `toReal` of `ℝ≥0∞`-valued limits.

## ii. Key results

- `Filter.Tendsto_inv_nat_mul_div_real` : `x⁻¹ * (x / m)` tends to `1 / m`.
- `ENNReal.tendsto_toReal_iff_of_eventually_ne_top` : `toReal` convergence under eventual
  finiteness.
...
theorem Filter.Tendsto_inv_nat_mul_div_real (m : ℕ) :
  ...
  --Thanks aristotle!
  -- This simplifies to $\lim_{x \to \infty} \frac{\lfloor x / m \rfloor}{x} = \frac{1}{m}$ because
  -- the floor function grows asymptotically like $x / m$.
    ...
    have h_floor_bound : ∀ x : ℕ, x > 0 → (Nat.floor (x / m : ℝ) : ℝ) / x ≥ (1 / m - 1 / x) ∧
      ...
      · rename_i h
        ...
        simp_all only [gt_iff_lt, CharP.cast_eq_zero, div_zero, Nat.floor_zero, zero_div, one_div,
          zero_sub, ge_iff_le, Left.neg_nonpos_iff, inv_nonneg, Nat.cast_nonneg, le_refl, and_self]
      · intro x a
        simp_all only [ne_eq, gt_iff_lt, one_div, ge_iff_le, tsub_le_iff_right]
        apply And.intro
        · rw [ inv_eq_one_div, div_add', div_le_div_iff₀ ] <;> first
            | positivity
  ...
  -- By definition of floor function, we know that ⌊(x : ℝ) / m⌋₊ is the greatest integer less than
  -- or equal to (x : ℝ) / m.
