...
theorem inv_antitone (hA : A.mat.PosDef) (h : A ≤ B) : B⁻¹ ≤ A⁻¹ := by
  ...
  have hB_inv : B.mat⁻¹ = A.mat⁻¹ -
      A.mat⁻¹ * C.conjTranspose * (1 + C * A.mat⁻¹ * C.conjTranspose)⁻¹ * C * A.mat⁻¹ := by
    have hB_inv : (A.mat + C.conjTranspose * C)⁻¹ = A.mat⁻¹ -
        A.mat⁻¹ * C.conjTranspose * (1 + C * A.mat⁻¹ * C.conjTranspose)⁻¹ * C * A.mat⁻¹ := by
      have hB_inv : (A.mat + C.conjTranspose * C) * (A.mat⁻¹ -
          A.mat⁻¹ * C.conjTranspose * (1 + C * A.mat⁻¹ * C.conjTranspose)⁻¹ * C * A.mat⁻¹) = 1 := by
  ...
  -- Since $(1 + C * A⁻¹ * C.conjTranspose)$ is positive definite, its inverse is positive definite.
...
lemma Real.integral_inv_sub_inv_finite (x T : ℝ) (hx : 0 < x) (hT : 0 < T) :
  ...
  · rw [ ← Real.log_mul, intervalIntegral.integral_deriv_eq_sub' ];
    ...
    · exact continuousOn_of_forall_continuousAt fun t ht => ContinuousAt.div continuousAt_const
        continuousAt_id ( by cases Set.mem_uIcc.mp ht <;> linarith );
    · exact funext fun x => by simp [ div_eq_inv_mul ];
    · exact continuousOn_of_forall_continuousAt fun t ht => ContinuousAt.div continuousAt_const
        continuousAt_id <| by cases Set.mem_uIcc.mp ht <;> linarith;
...
lemma Real.tendsto_log_div_add_atTop (x : ℝ) :
  ...
  -- Divide the numerator and the denominator by $b$ and take the limit as $b$ approaches infinity.
  suffices h_div : Filter.Tendsto (fun b => Real.log ((1 / b + 1) / (x / b + 1)))
  ...
  have h_integrable : ContinuousOn (fun t : ℝ => (1 + t)⁻¹ • (1 : HermitianMat d 𝕜)) (Set.Icc 0 T) ∧
    ...
    · refine' ContinuousOn.comp ( show ContinuousOn ( fun m : HermitianMat d 𝕜 => m⁻¹ )
      ...
      · intro t ht;
        ...
        · intro v hv_ne_zero
          have h_pos : 0 < star v ⬝ᵥ x.mat.mulVec v +
              t * star v ⬝ᵥ (1 : Matrix d d 𝕜).mulVec v := by
    ...
    · have h_cont : ContinuousOn (fun t : ℝ => (y + t • 1 : Matrix d d 𝕜)⁻¹) (Set.Icc 0 T) := by
        ...
        have h_cont_inv : ContinuousOn (fun t : ℝ => (y + t • 1 : Matrix d d 𝕜)⁻¹)
            (Set.Icc 0 T) := by
          have h_cont_det : ContinuousOn (fun t : ℝ => (y + t • 1 : Matrix d d 𝕜).det)
              (Set.Icc 0 T) := by
            fun_prop
          have h_cont_adj : ContinuousOn (fun t : ℝ => (y + t • 1 : Matrix d d 𝕜).adjugate)
              (Set.Icc 0 T) := by
            fun_prop;
          simp_all [ Matrix.inv_def ];
...
/-- The finite-integral (truncated at `T`) approximation of the matrix logarithm of `x`. -/
...
/-- The scalar finite-integral (truncated at `T`) approximation of `Real.log u`. -/
...
theorem logApprox_eq_log_add_error
  ...
  have h_logApprox : ∫ t in (0)..T, ((1 + t)⁻¹ • (1 : HermitianMat d 𝕜) - (x + t • 1)⁻¹) =
...
lemma tendsto_cfc_log_div_add_atTop (x : HermitianMat d 𝕜) :
  ...
  have h_sum : Filter.Tendsto (fun T : ℝ => ∑ i, Real.log ((1 + T) / (x.H.eigenvalues i + T)) •
    ...
    convert Filter.Tendsto.smul_const
      ( Real.tendsto_log_div_add_atTop ( x.H.eigenvalues i ) ) _ using 1;
...
lemma inv_convex {x y : HermitianMat d 𝕜} (hx : x.mat.PosDef) (hy : y.mat.PosDef)
  ...
  have h_pos_semidef :
    (Matrix.fromBlocks (a • x.mat + b • y.mat) (1 : Matrix d d 𝕜) (1 : Matrix d d 𝕜)
      ...
      -- Since $a + b = 1$, we can use the fact that the block matrix
      -- $\begin{pmatrix} A & I \\ I & A^{-1} \end{pmatrix}$ is positive semidefinite for any
      -- positive definite $A$.
      have h_block_pos : ∀ A : Matrix d d 𝕜, A.PosDef →
          (Matrix.fromBlocks A 1 1 A⁻¹).PosSemidef := by
        intro A hA
        have h_block_pos : (Matrix.fromBlocks A (1 : Matrix d d 𝕜) (1 : Matrix d d 𝕜)
            (A⁻¹)).PosSemidef := by
          have h_inv_pos : A⁻¹.PosSemidef := by
            exact hA.inv.posSemidef
          have h_block_pos : (Matrix.fromBlocks A (1 : Matrix d d 𝕜) (1 : Matrix d d 𝕜) (A⁻¹)) =
              (Matrix.fromBlocks 1 0 A⁻¹ 1) * (Matrix.fromBlocks A 0 0 (A⁻¹ - A⁻¹ * A * A⁻¹)) *
                (Matrix.fromBlocks 1 A⁻¹ 0 1) := by
            simp [ Matrix.fromBlocks_multiply ];
            have := hA.det_pos;
            exact ⟨ by rw [ Matrix.mul_nonsing_inv _
              ( show IsUnit A.det from isUnit_iff_ne_zero.mpr this.ne' ) ], by
      ...
      have h_convex : Matrix.PosSemidef ((a • Matrix.fromBlocks (x.mat) (1 : Matrix d d 𝕜)
          (1 : Matrix d d 𝕜) (x.mat)⁻¹) +
          (b • Matrix.fromBlocks (y.mat) (1 : Matrix d d 𝕜) (1 : Matrix d d 𝕜) (y.mat)⁻¹)) := by
...
lemma integrable_inv_shift {A : HermitianMat d 𝕜} (hA : A.mat.PosDef) (b : ℝ) (hb : 0 ≤ b) :
  ...
  -- Since A is positive definite, for any t ≥ 0, A + t • 1 is also positive definite,
  -- hence invertible.
...
theorem logApprox_concave {n 𝕜 : Type*} [Fintype n] [DecidableEq n] [RCLike 𝕜]
  ...
  have h_integrable {z : HermitianMat n 𝕜} : z.mat.PosDef →
    ...
    exact ContinuousOn.integrableOn_Icc ( by
        exact continuousOn_of_forall_continuousAt fun t ht => ContinuousAt.smul
          ( ContinuousAt.inv₀ ( continuousAt_const.add continuousAt_id ) ( by linarith [ ht.1 ] ) )
          continuousAt_const ) |> fun h => h.mono_set ( Set.Ioc_subset_Icc_self );
  ...
  have h_integral_mono :
    ...
    · exact ( (h_integrable hx).1.smul a |> fun h => h.add ( (h_integrable hy).1.smul b ) ) |>
        fun h => h.mono_measure
          ( MeasureTheory.Measure.restrict_mono ( Set.Ioc_subset_Ioc le_rfl le_rfl ) le_rfl );
  ...
  rw [ intervalIntegral.integral_add ( by exact (h_integrable hx).smul a )
      ( by exact (h_integrable hy).smul b ),
