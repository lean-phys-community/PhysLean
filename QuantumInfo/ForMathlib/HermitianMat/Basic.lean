...
This file defines `HermitianMat n α`, the subtype of `Matrix n n α` that are Hermitian, and develops
its basic algebraic and order structure: the coercion to matrices, conjugation, the associated
linear map, eigenspaces, kernel, support, and diagonal Hermitian matrices.

## ii. Key results

- `HermitianMat` : the type of Hermitian matrices, as a subtype.
- `HermitianMat.conj` : conjugation of a Hermitian matrix by a (possibly rectangular) matrix.
- `HermitianMat.ker`, `HermitianMat.support` : the kernel and support submodules.
- `HermitianMat.eigenspace`, `HermitianMat.diagonal` : eigenspaces and diagonal Hermitian matrices.
...
/-- The `μ`-eigenspace of a Hermitian matrix `A`, as a submodule of Euclidean space. -/
...
variable (𝕜) in
/-- The Hermitian matrix with real diagonal entries given by `f` and zero off-diagonal entries. -/
  ...
  · intro x hx;
    ...
    obtain ⟨y, hy⟩ : ∃ y ∈ LinearMap.range (Matrix.toEuclideanLin (M.conjTranspose)),
      ...
      obtain ⟨y1, y2, hy1, hy2, hy⟩ : ∃ y1 y2 : EuclideanSpace 𝕜 d,
        ...
        exact ⟨ _, _, h_decomp y |> Classical.choose_spec |> And.left,
