...
theorem posSemidef_iff_spectrum_nonneg [DecidableEq n] (A : HermitianMat n 𝕜) :
    0 ≤ A ↔ ∀ x ∈ spectrum ℝ A.mat, 0 ≤ x := A.posSemidef_iff_spectrum_Ici
...
omit [Fintype n] in
open MatrixOrder in
theorem posSemidef_to_nonneg {A : Matrix n n 𝕜} (hA : A.PosSemidef) : 0 ≤ A := hA.nonneg
...
theorem mat_posDef_to_pos [Nonempty n] (hA : A.mat.PosDef) : 0 < A := posDef_to_pos hA
