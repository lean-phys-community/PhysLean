...
/-!
# Minimax theorems for bilinear forms

## i. Overview

This file derives the von Neumann minimax theorem at the generality needed downstream, from Sion's
minimax theorem, together with supporting `fun_prop` lemmas about (quasi)convexity and lower/upper
semicontinuity of linear maps and bilinear forms.

## ii. Key results

- `minimax` : for convex compact nonempty sets and a bilinear map, inf-sup equals sup-inf.
- `LinearMap.BilinForm.minimax` : von Neumann's minimax theorem specialized to bilinear forms.
- `LinearMap.BilinForm.minimax'` : the same with the order of the inf/sup arguments flipped.
...
theorem _root_.IsCompact.exists_isMinOn_lowerSemicontinuousOn {α β : Type*}
  ...
  --Thanks Aristotle
  -- By the Extreme Value Theorem for lower semicontinuous functions on compact sets, there exists
  -- x in s such that f(x) is the minimum value of f on s.
  have h_extreme : ∃ x ∈ s, ∀ y ∈ s, f x ≤ f y := by
    ...
    obtain ⟨t, ht⟩ : ∃ t : Finset β, (∀ x ∈ t, x ∈ s) ∧ s ⊆ ⋃ x ∈ t, U x ∩ s := by
      ...
      obtain ⟨t, ht⟩ : ∃ t : Finset β, (∀ x ∈ t, x ∈ s) ∧ s ⊆ ⋃ x ∈ t, U x := by
        exact hs.elim_nhds_subcover U fun x hx =>
          IsOpen.mem_nhds ( hU x hx |>.1 ) ( hU x hx |>.2.1 );
  ...
  -- By definition of IsMinOn, we show that for all y in s, f(x) ≤ f(y), which h_extreme provides.
...
/-- Convenience form of `LinearMap.BilinForm.minimax` with the inf/sup arguments to `f` flipped. -/
theorem LinearMap.BilinForm.minimax'
    {M : Type*} [NormedAddCommGroup M] [Module ℝ M] [ContinuousAdd M] [ContinuousSMul ℝ M]
  [FiniteDimensional ℝ M]
  (f : LinearMap.BilinForm ℝ M)
